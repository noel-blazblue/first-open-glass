...
import com.glass.dining.phone.place.AmapPlaceSearch
...
class NavigationToolProvider(private val world: PhoneWorld) {
    ...
    private fun searchNearby(args: JSONObject): String {
        ...
        if (world.amapKey().isBlank()) {
            return error("no_key", "没配地点搜索密钥，在 ai.env 写 AMAP_WEB_KEY", retryable = false)
        }
        val result = AmapPlaceSearch.around(origin, keyword, world.amapKey(), radius)
        if (!result.ok) {
            return error(result.error.ifBlank { "amap_error" }, result.message.ifBlank { "公开地点搜索失败" })
        }
    ...
    private fun stopNav(): String {
        val was = world.session.navigating
        world.dismiss(DismissReason.STOP_NAV)
        return JSONObject()
            .put("ok", true)
            .put("stopped", was)
