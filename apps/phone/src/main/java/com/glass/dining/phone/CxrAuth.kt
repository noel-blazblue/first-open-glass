...
object CxrAuth {
    ...
    fun hasCamera(): Boolean {
        return AuthorizationHelper.hasGlassPermission(GlassPermission.CAMERA)
    }

    fun hasMicrophone(): Boolean {
        return AuthorizationHelper.hasGlassPermission(GlassPermission.MICROPHONE)
    }
        ...
        return when (result) {
            is AuthResult.AuthSuccess -> {
                ...
                if (token.isBlank()) {
                    ...
                    Log.i(TAG, "cxr token len=${token.length} camera=${hasCamera()}")
                    CxrLinkHost.connect(activity.application, token)
                }
            }
    ...
    private fun connectSaved(activity: Activity): String? {
        ...
        Log.i(TAG, "auth skipped, connecting with saved token camera=${hasCamera()}")
        return CxrLinkHost.connect(activity.application, saved)
    }
