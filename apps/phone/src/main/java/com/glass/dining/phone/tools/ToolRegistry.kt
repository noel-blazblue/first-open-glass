...
class ToolRegistry {
    ...
    fun execute(name: String, args: JSONObject): ToolResult {
        ...
        val callArgs = when (name) {
            "look_store" -> args.apply { if (!has("purpose")) put("purpose", "store") }
            "read_menu" -> args.apply { if (!has("purpose")) put("purpose", "menu") }
            "read_sign" -> args.apply { if (!has("purpose")) put("purpose", "observe") }
            else -> args
        }
        return try {
            val raw = exec(callArgs)
