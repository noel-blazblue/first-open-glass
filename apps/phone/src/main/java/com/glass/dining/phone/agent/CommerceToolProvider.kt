...
class CommerceToolProvider(private val world: PhoneWorld) {
    ...
    private fun listCoupons(): String {
        ...
        world.publishSkillCard(card, ActiveSkill.COUPON, card.wait.ifBlank { "这店暂无券" }, "list_coupons")
        return JSONObject()
            .put("ok", true)
    ...
    private fun redeem(args: JSONObject): String {
        ...
        if (!args.optBoolean("confirm", false)) {
            ...
            world.publishSkillCard(card, ActiveSkill.COUPON, "确认后才核销${picked.title}", "redeem_coupon")
            return JSONObject()
                .put("ok", true)
                .put("need_confirm", true)
                .put("mock", true)
        ...
        world.publishSkillCard(card, world.session.activeSkill, receipt.message, "redeem_coupon")
        return JSONObject()
            .put("ok", receipt.ok)
            .put("mock", true)
    ...
    private fun checkout(args: JSONObject): String {
        if (args.optBoolean("confirm", false)) {
            ...
            world.publishSkillCard(card, world.session.activeSkill, "已付${done.amount}元，这是演示", "checkout")
            return JSONObject()
                .put("ok", true)
                .put("mock", true)
                .put("paid", true)
